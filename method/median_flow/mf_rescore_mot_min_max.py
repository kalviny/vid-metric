    ...
    def update(self, dets, of_fw, of_bw, fr_name):
        ...
        for t in range(len(self.trackers) - 1, -1, -1):
            if (t in unmatched_trks):
                if (self.trackers[t][-1].bbox.pos[4] <= 0.9):
                    f_trk.append(t)
                else:
                    trk = self.trackers[t][-1]
                    new_bbox = Bbox()
                    new_bbox.pos = np.hstack((trk.bbox.opt_flow, trk.bbox.pos[4] * 0.9))
        ...
        with open(os.path.join(out_dir, 'det.txt'), 'w') as f:
            ...
            for t in all_traj:
                ...
                for item in now_traj:
                    ...
                    mi = min(item.bbox.pos[4], mi)
                    mx = max(item.bbox.pos[4], mx)
                    #sc += item.bbox.pos[4]
                #print(len(now_traj))
                ...
                for item in now_traj:
                    ...
                    bbox[item.idx, 4] = 1. * (bbox[item.idx, 4] - mi) / (mx - mi + 1)
