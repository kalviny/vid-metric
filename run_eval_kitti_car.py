    ...
    for c in cls:
        ...
        for i in range(len(vid_list)):
            ...
            for x in range(len(key)):
                ...
                _bbox = np.hstack((val[x][:, 2:], val[x][:, 1:2]))
            ...
            fout_st = open(os.path.join(st_out_dir, '{}_{}.txt'.format(vid_name, c)), 'w')

            with open(os.path.join(out_dir, '{}_{}.txt'.format(vid_name, 'draw')), 'w') as f:
                for th in thr:
                    ...
                    _err, _c, _r = EvalAssn(anno, det_traj, vid_name, sc, th)
                    '''
                    for item in _rec_ls:
                        sc_thr = item[1]
                        x.append(item[0])
